...
import { getStraightPath } from '../connectors/connectorRouting';
...
export class LineTool extends BaseTool {
  ...
  onMouseDown(opt, baseOptions) {
    ...
    objects.forEach(obj => {
      ...
      for (const [name, pt] of Object.entries(anchors)) {
        if (name === 'center') continue; // Anchor to edges only for connectors
  ...
  onMouseMove(opt) {
    ...
    const pathData = getStraightPath({ x: sx, y: sy }, { x: ex, y: ey });
    ...
    if (this.sourceObj && this.targetObj && this.sourceObj !== this.targetObj) {
      import('../connectors/connectorBindings').then(({ ConnectorBindings }) => {
        import('../connectors/connectorRouting').then(({ RoutingModes }) => {
          ConnectorBindings.bind(this.shape, this.sourceObj, this.targetObj, RoutingModes.STRAIGHT);
